                      ...
                      std::string &pbf_buffer)
{
    ...
    protozero::pbf_writer tile_writer{pbf_buffer};
    {
        ...
        // OSM Node tile layer
        {
            ...
            std::unordered_set<NodeID> internal_node_set;
            for (const auto &edge : edges)
            {
                internal_node_set.insert(edge.u);
                internal_node_set.insert(edge.v);
            }
            std::vector<NodeID> internal_nodes;
            for (const auto internal_node : internal_node_set)
            {
